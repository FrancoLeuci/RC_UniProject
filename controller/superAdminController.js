...
async function addSuperAdmin(req,res,next){
    ...
    try{
        const isSuperAdmin=await User.findOne({_id: requestUserId, role: 'super-admin'})
        if(!isSuperAdmin) throw new HttpError("You are not a super admin.",403)

        const userToPromote=await User.findById(userToPromoteId)
...
async function portalDeletionResponse(req,res,next){
    ...
    try{
        const isSuperAdmin=await User.findOne({_id:sAdminId,role:"super-admin"})
        ...
        if(action === 'accepted'){
            ...
            await Promise.all(portal.admins.map(async a => {
                //eliminazione dal User
                const adminAccount = await User.findById(a)
                const index = adminAccount.portals.indexOf(portal._id)
                adminAccount.portals.splice(index,1)
                await adminAccount.save()

                //eliminazione dal Author
                const adminFull = await Author.findOne({basicCorrespondent: a})

                //notifiche x tutti gli admin di portale
                const notification = await Notification.findOne({receiver: a})
                if(notification){
                    notification.backlog.push(`${portal.name} has been deleted. Following this, all links with Expositions were cut. `)
                    await notification.save()
                } else {
                    await Notification.create({
                        receiver: a,
                        backlog: `${portal.name} has been deleted. Following this, all links with Expositions were cut. `
                    })
                }
            }))
            ...
            await Promise.all(portal.members.map(async m => {
                //eliminazione dal User
                const adminAccount = await User.findById(m)
                const index = adminAccount.portals.indexOf(portal._id)
                adminAccount.portals.splice(index,1)
                await adminAccount.save()

                //eliminazione dal Author
                const adminFull = await Author.findOne({basicCorrespondent: m})
    ...
    try{
        ...
        if(action==='accepted'){
            ...
            await Promise.all(request.formFields.adminList.map(async admin => {
                ...
                const userFull=await Author.findOne({basicCorrespondent:admin})
                if(!userFull){
...
async function userDeletionResponse(req,res,next){
    ...
    try{
        ...
        if(action==="accepted"){
            //NuovoAccount10
            const userToDelete = await User.findById(request.sender).populate({path:"portals", populate:{path:"linkedExpositions",}}) //nested populate
            ...
            const userToDeleteFull = await Author.findOne({basicCorrespondent: userToDelete._id}).populate({path:"expositions",populate:[{path:"portal"},{path:"authors.userId"}]})

            //elimazione dei set a lui condivisi
            const setSharedWithUserToDelete=await Set.find({otherUserPermissions:{user:userToDelete._id}}).populate("mediaList")
...
async function fullAccountResponse(req,res,next){
    ...
    try{
        ...
        if(action==="accepted"){
            const newFull=await Author.create({
...
async function youShallNotPass(req,res,next){
    ...
    try{
        ...
        const expo=await Exposition.findOneById(expoId);
        if(!expo)throw new HttpError("The exposition you are trying to modify doesn't exist.",404)

        if(!expo.published){
            throw new HttpError("This exposition hasn't been issued yet.",400)
        }else{
            expo.published=false;
            await expo.save();
            const creator=expo.authors.find(a=>a.role==="creator");
            const notification = await Notification.findOne({receiver: creator.userId})
            if(notification){
                notification.backlog.push(`Super-admin ${isSUperAdmin.realName} has decided that your exposition ${expo.title} can't be public yet. Contact the super admin for more informations.`)
                await notification.save()
            } else {
                await Notification.create({
                    receiver: creator.userId,
                    backlog: `Super-admin ${isSUperAdmin.realName} has decided that your exposition ${expo.title} can't be public yet. Contact the super admin for more informations.`
                })
            }
            res.status(200).send("Exposition is private now.")
        }
    }catch(err){
        next(err)
    }
